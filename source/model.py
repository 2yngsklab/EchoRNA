...
class RNP_adapter(nn.Module):
    def __init__(self,
        ...
        """
        Args:
            embed_dim (int): width of the RNA-FM token representation (default: 640).
            trans_dim (int): hidden dimension of the cross-attention transformer.
            num_heads (int): number of heads in the cross-attention layers.
            num_trans_layers (int): number of stacked transformer blocks.
            mlp_ratio (int): expansion factor of the transformer feed-forward hidden dim.
            time_dim (int): dimension of the diffusion time-step embedding.
            dropout_rate (float): dropout probability inside the transformer blocks.
            lora_config (LoRA_Config): LoRA configuration; if None the RNA-FM encoder is frozen instead of adapted.
            lora_bias (str): which biases LoRA keeps trainable ('none' | 'all' | 'lora_only').
            gvp_layers (int): number of GVP-GNN layers in the protein structure encoder.
            node_feature (list): protein node features to use, a subset of
                {"one-hot", "esm-2", "esm-if"} (default: all three). The protein input
                node dimension is the sum of the selected feature widths: one-hot = 21
                (amino acids incl. unknown), esm-2 = 1280, esm-if = 512.
            device (str): device on which to place the RNA-FM encoder.
        """
        ...
        self.node_in_dim = (node_in_feat, 4)
        self.node_h_dim = (256, 128)
        self.edge_in_dim = (48, 1)
        self.edge_h_dim = (128, 1)
        self.num_layers = gvp_layers
