...
class GVP(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.vi:
            ...
            vh = self.wh(v) # vh = (N, 3, h_dim)
            vn = _norm_no_nan(vh, axis=-2) # (N, 1, h_dim)
...
class GVPConv(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        x_s, x_v = x
        src, dst = edge_index
        message = self.message(x_s, x_v.contiguous().view(x_v.shape[0], x_v.shape[1] * 3), edge_index,
                               edge_attr=edge_attr)
        if self.aggr == "mean":
            aggr_message = scatter_mean(message, dst, dim=0, dim_size=x_s.size(0))
        elif self.aggr == "add":
            aggr_message = scatter_add(message, dst, dim=0, dim_size=x_s.size(0))

        return _split(aggr_message, self.vo)

    def message(self, s, v, edge_index, edge_attr):
        s_i = s[edge_index[0]]
        s_j = s[edge_index[1]]
...
class GVPConvLayer(nn.Module):
    ...
    def __init__(
            self,
            node_dims, # (n_scalar, n_vector)
            edge_dims, # (n_scalar, n_vector)
            n_message=3,
            n_feedforward=2,
            drop_rate=.1,
...
if __name__ == "__main__":
    from main import *
    from GVP import GVPConv, GVP, GVPConvLayer
    ...
    rna = list(set(complex) - set(protein))
    
    print("protein:", protein)
    print("rna:", rna)
    
    rna_pdbs = [os.path.join(complex_dir, i) for i in rna]
    protein_pdbs = [os.path.join(complex_dir, i) for i in protein]

    data = PDBtoData(rna_pdbs[0], "RNA", 16, 16, 16)
    print("node_s shape", data["node_s"].shape)
    print("node_v shape", data["node_v"].shape)
    print("edge_s shape", data["edge_s"].shape)
    print("edge_v shape", data["edge_v"].shape)
    ...
    # GVP
    model = GVP((4, 4), (128, 128))
    print(model)
    out_s, out_v = model((V_s, V_v))
    print(out_s.shape)
    print(out_v.shape)
    
    # GVPConv
    model = GVPConv(in_dims=(4, 4), out_dims = (128, 128), edge_dims=(32, 1))
    print(model)
    out_s, out_v = model((V_s, V_v), edge, (E_s, E_v))
    print(out_s.shape)
    print(out_v.shape)
    
    # GVPConvLayer
    model = GVPConvLayer(node_dims=(4, 4), edge_dims = (32, 1))
    print(model)
    out_s, out_v = model((V_s, V_v), edge, (E_s, E_v))
    print(out_s.shape)
    print(out_v.shape)