...
class our_diffusion(nn.Module):
    ...
    def generate_RDMSampling(self, batch, 
        ...
        return decoder_out['output_tokens'], {'output_scores': decoder_out['output_scores'], 'history': decoder_out['history']}

# #Daniil's vanilla code
#     def vanilla_inference(self, protein, rna, num_steps=100, confidence_threshold=0.5, 
#                          random_masking_probability=0.6, mask_token_id=24, distance_bias = None, device = "cuda:0"):

#         noisy_tokens, _ = self.initialize_output_tokens(rna, encoder_out={}, partial_masks=None) 
#         mask = torch.zeros_like(noisy_tokens, dtype=torch.bool, device=device)

#         for t in reversed(range(num_steps)):
#             time_step = torch.full((noisy_tokens.size(0),), t, dtype=torch.long).to(device)
#             with torch.no_grad():
#                 logits, _, weights = self.model(protein, {"input_ids":noisy_tokens},
#                                                 time_step, num_timesteps = self.num_diff_step, distance_bias=distance_bias)
...
#             confident_tokens = max_probs > confidence_threshold
#             noisy_tokens = torch.where(confident_tokens, predicted_tokens, noisy_tokens)
            
#             if t > 0.3 * num_steps:
#                 random_mask = torch.rand_like(noisy_tokens, dtype=torch.float, device=device) < random_masking_probability
#                 noisy_tokens = torch.where(random_mask, mask_token_id, noisy_tokens)
            
#         denoised_tokens = noisy_tokens.squeeze()
#         return denoised_tokens, weights, logits
    
    
#     def topk_gumbel_inference(
#         self, protein, rna, num_steps=100,
#         mask_token=24, distance_bias=None, k_tokens = 5, device="cuda:0",
#         tau=1.0, noise_scale=1.0, unmasked_ids=None, mask_prob=0.1
#     ):
#         """
#         Top-k Sampling combined with Gumbel-Max Trick for diffusion-based language models.
        
#         Args:
#             model: The language model being used.
#             protein: Protein input data.
#             noisy_tokens (torch.Tensor): Noisy input tokens.
#             num_steps (int): Number of denoising steps.
#             k (int): Number of top candidates for Top-k sampling.
#             confidence_threshold (float): Threshold for token confidence.
#             mask_token_id (int): Token ID for masking.
#             distance_bias: Optional distance bias for the model.
#             device: Device for computation (CPU/GPU).
        
#         Returns:
#             denoised_tokens (torch.Tensor): Final denoised token sequence.
#             tot_weights (list): Final layer weights from the model.
#         """
#         self.model.eval()
#         noisy_tokens, _ = self.initialize_output_tokens(rna, encoder_out={}, partial_masks=None) 
#         print(noisy_tokens)
...
#             for t in reversed(range(num_steps)):
#                 if t < 0.25 * num_steps: mask_prob = 0
#                 else: mask_prob=0.1
#                 time_step = torch.full((noisy_tokens.size(0),), t, dtype=torch.long).to(device)
#                 with torch.no_grad():
#                     logits, _, weights = self.model(protein, {"input_ids":noisy_tokens},
#                                                     time_step, num_timesteps = self.num_diff_step, distance_bias=None)
...
#                 unmasked_ids = noisy_tokens!=self.mask_id
#                 noisy_tokens = sample_with_adjusted_gumbel(logits, 
#                                                            tau=tau, 
...
# def generate_initial_noise(batch_size, sequence_length, mask_token_id, device):
#     initial_noise = torch.full((batch_size, sequence_length), mask_token_id).to(device)
#     return initial_noise


# def sample_with_adjusted_gumbel(logits, tau=1.0, noise_scale=4.0, mask_token=24, k_tokens = 5, unmasked_ids = None, mask_prob=0.1):
#     """
#     Adds Gumbel noise with scaling to logits and applies softmax for sampling.

#     Args:
#         logits (torch.Tensor): Input logits of shape [N, len, k].
#         tau (float): Temperature parameter. Higher values flatten probabilities.
#         noise_scale (float): Scaling factor for Gumbel noise.
#     Returns:
#         torch.Tensor: Probabilities after Gumbel-Softmax sampling, shape [N, len, k].
#     """
#     if unmasked_ids == None:
#         unmasked_ids = torch.zeros([logits.size(0), logits.size(1)], dtype=torch.bool)
#     gumbel_noise = -torch.log(-torch.log(torch.rand_like(logits) + 1e-20) + 1e-20)
#     gumbel_noise *= noise_scale
#     noisy_logits = logits + gumbel_noise
#     scaled_logits = noisy_logits / tau
#     top_k_values, _ = torch.topk(scaled_logits, k=5, dim=-1)
#     min_top_k_values = top_k_values[..., -1, None] 
#     scaled_logits = torch.where(scaled_logits >= min_top_k_values, scaled_logits, torch.tensor(float('-inf'), device=logits.device))
    
#     probabilities = F.softmax(scaled_logits, dim=2)

#     mask = unmasked_ids.unsqueeze(-1)#
#     masked_probs = torch.where(mask, torch.tensor(float('-inf'), device=logits.device), probabilities)
#     max_probs, _ = masked_probs.max(dim=2, keepdim=True)
#     # max_probs, _ = probabilities.max(dim=2, keepdim=True)
#     _, indices = torch.topk(max_probs, k=k_tokens, dim=1)
#     tokens = torch.multinomial(probabilities.view(-1, probabilities.size(-1)), 1).view(probabilities.size(0), probabilities.size(1))
    
#     batch_size, seq_length = tokens.size()
#     binary_mask = torch.zeros_like(tokens, dtype=torch.bool)
    
#     for batch_idx, idx_list in enumerate(indices.squeeze(-1)):
#         binary_mask[batch_idx, idx_list.flatten()] = True
#     binary_mask += unmasked_ids
#     masked_tokens = torch.where(binary_mask, tokens, mask_token)
#     random_mask = (torch.rand_like(masked_tokens, dtype=torch.float) < mask_prob)
#     final_tokens = torch.where(random_mask, mask_token, masked_tokens)
#     return final_tokens
