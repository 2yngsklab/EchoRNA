...
class Echo_diffusion(nn.Module):
    ...
    def get_non_special_sym_mask(self, rna_ids, partial_masks=None):
        """
        Build a boolean mask selecting the positions that may be (re)generated.

        Despite the historical "non special" name, only PAD positions are excluded
        here (BOS/EOS are *not* masked out). When ``partial_masks`` is given, those
        positions are additionally excluded.

        Args:
            rna_ids: RNA token IDs tensor.
            partial_masks: Optional boolean tensor; its True positions are excluded.

        Returns:
            Boolean mask where True marks a non-PAD, non-partial-masked position.
        """
    ...
    def sample_noise(self, x_0, t1, maskable_mask, scheduler='cosine'):
        ...
        Args:
            ...
            scheduler (str): Noise scheduler, either 'cosine' (default) or 'linear'
        ...
        if scheduler == 'cosine':
            alpha_t = 0.5 * (1 - torch.cos(math.pi * t1 / self.num_diff_step))
        else:
            alpha_t = t1 / self.num_diff_step
    ...
    def initialize_output_tokens(self, batch, partial_masks=None, **kwargs):
        """
        Initialize output tokens for generation by masking every maskable position
        (see get_non_special_sym_mask for what counts as maskable).
    ...
    def _reparam_decoding(
        ...
        """
        Perform one reparameterized decoding step.

        Decides which positions keep their current predictions and which are re-masked
        for the next step. The lowest-scoring positions (by a linear or cosine rate
        schedule derived from ``t`` and ``max_step``) are set to ``noise``; the
        remaining previously-masked positions (``xt_neq_x0``) adopt cur_tokens.

        The ``decoding_strategy`` string must have the form
        "reparam-<condition>-<topk_mode>-<schedule>".

        Args:
            output_tokens: Previous-step token tensor [B, N].
            output_scores: Previous-step score tensor [B, N].
            cur_tokens: Current-step predicted tokens [B, N].
            cur_scores: Current-step prediction scores [B, N].
            decoding_strategy (str): Reparameterized-decoding config string.
            xt_neq_x0: Bool tensor [B, N]; True where a position is still masked.
            non_special_sym_mask: Bool tensor [B, N]; True for non-PAD positions.
            t (int): Current step index.
            max_step (int): Total number of diffusion steps.
            noise: MASK token id (int/float) or a [B, N] token tensor.
            pretrain (bool): Unused; kept for interface compatibility.

        Returns:
            tuple: (new_xt_neq_x0 [B, N], output_tokens [B, N], output_scores [B, N])
        """
        ...
        # When condition is 'uncond', not_v1_t equals not_v2_t, so new_xt_neq_x0 always equals not_v1/v2_t
    ...
    def forward_decoder(self, prev_decoder_out, encoder_out=None, need_attn_weights=False, partial_masks=None,
                        sampling_strategy='gumbel', resample=True):
        """
        Run one denoising step: predict tokens for the currently masked positions.

        The model is queried at the current step, the MASK logit is suppressed, and
        new tokens are sampled (categorical for 'vanilla', Gumbel for 'gumbel'). The
        sampled values are scattered back only into the maskable positions.

        Args:
            prev_decoder_out (dict): State from the previous step (protein, output
                tokens/scores, step, max_step, temperature, history, score_history).
            encoder_out: Unused; kept for interface compatibility.
            need_attn_weights (bool): Unused; attention weights are not returned.
            partial_masks: Optional positions to keep fixed (see get_non_special_sym_mask).
            sampling_strategy (str): 'vanilla' (categorical) or 'gumbel' (Gumbel noise,
                with optional resampling of over-represented tokens).
            resample (bool): For 'gumbel', whether to apply resample_conditional.

        Returns:
            dict: Updated decoder state with the new output_tokens/scores and the
            decremented step.
        """
        ...
        elif sampling_strategy == 'gumbel':
            noise_scale = 1.0
            _tokens, _scores = stochastic_sample_from_categorical(logits, temperature=0.0, noise_scale=noise_scale)
            if resample:
                self.resample_conditional(protein, _tokens, _scores, ratio=0.8, scale=1.0, step=step)
        else:
            raise NotImplementedError
    ...
    def generate_RDMSampling(self, batch, 
        ...
        """
        Generate RNA sequences with reparameterized discrete-diffusion sampling.

        Starting from a fully masked sequence, this iterates the backward diffusion
        process for ``self.num_diff_step`` steps: each step calls forward_decoder to
        sample tokens and then _reparam_decoding to decide which positions are kept
        versus re-masked for the next step.

        Args:
            batch (tuple): ``(rna, protein)`` where ``rna`` provides ``input_ids``
                (the initial template, typically all-MASK) and ``protein`` is the
                featurized protein graph used as conditioning.
            use_tqdm (bool): Unused; kept for interface compatibility.
            temperature (float | None): Sampling temperature for the 'vanilla' strategy.
            partial_masks: Optional positions to keep fixed throughout sampling.
            sampling_strategy (str): 'vanilla' (categorical) or 'gumbel' (default).
            decoding_strategy (str): Reparameterized-decoding schedule of the form
                "reparam-<cond>-<topk>-<schedule>". NOTE: this argument is currently
                overridden internally — the reparam step always runs with
                'reparam-uncond-stochastic1.0-linear'.
            resample (bool): Passed to forward_decoder (gumbel resampling).

        Returns:
            tuple: ``(output_tokens, info)`` where ``output_tokens`` is the final
            [B, L] token tensor and ``info`` holds 'output_scores', 'history' and
            'score_history'.
        """
