...
training_start = time.time()  # start global timer
...
class EchoRNA(nn.Module):
    ...
    def step(self, batch, attention_bais=None, preservation_loss_ratio=0):
        ...
        if torch.isnan(loss):
            ...
            logging_output["accuracy"] = ((logits.argmax(-1) == target) & loss_mask).sum().item() / (
                        loss_mask.sum().item() + 1e-9)

        logging_output["preservation"] = ((logits.argmax(-1) == target) & ~loss_mask).sum().item() / (
                    (~loss_mask).sum().item() + 1e-9)
    ...
    def training_step(self, batch, attention_bais=None, step=None,
        ...
        self.model.train()
        loss, logging_output, logits, target, loss_mask, prs_loss = self.step(batch, attention_bais,
                                                                             preservation_loss_ratio)
        if preservation_loss_ratio != 0:
...
# -------------------------------------------------
# TRAINING SUMMARY (TIME & MEMORY) ─────────────────
# -------------------------------------------------
training_end = time.time()
elapsed_s = training_end - training_start
elapsed_min = elapsed_s / 60

# if device.type == "cuda" and torch.cuda.is_available():
peak_mem_gb = torch.cuda.max_memory_allocated(device) / (1024 ** 3)
print("\n[Training Summary]")
print(f"Total training time  : {elapsed_min:.2f} minutes ({elapsed_s:.1f} seconds)")
print(f"Peak GPU memory usage: {peak_mem_gb:.2f} GB")
# else:
#     print("\n[Training Summary]")
#     print(f"Total training time : {elapsed_min:.2f} minutes ({elapsed_s:.1f} seconds)")
#     print("Peak GPU memory usage: N/A (training on CPU)")
